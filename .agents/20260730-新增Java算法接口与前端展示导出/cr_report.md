...
本次实现覆盖全部需求项，代码结构清晰、分层合理（Controller→Service→Model）、前后端契约对齐严密、异常处理设计正确。问题修复阶段已修复此前评审轮次发现的全部 Blocker 问题。经对照实际代码逐文件独立复审，剩余 **0 个 Major** 和 **2 个 Minor**（可选优化），均不阻塞合并。
...
| 检查项 | 结论 | 证据 |
|--------|------|------|
| 后端 `ApiResult{code,message,data}` 与前端 `ApiResult<T>` 类型对齐 | ✅ 一致 | `ApiResult.java` L9-13 vs `demo.ts` L4-8 |
...
| 导出接口 Content-Disposition: attachment | ✅ 一致 | `DemoController.java` L101 export 方法 |
| CORS 暴露 Content-Disposition 头 | ✅ 一致 | `WebConfig.java` L23 `exposedHeaders("Content-Disposition")` |
...
### 3.2 ApiResult.java — ✅ 无问题（注释措辞可选优化）

泛型统一响应体 `ApiResult<T>`，含 `success(T)` / `error(int, String)` 工厂方法。`code=0` 成功，字段含 getter，可被 Jackson 正确序列化。

**实际代码验证**（43 行）：类仅暴露 getter（`getCode`/`getMessage`/`getData`），**无 setter**，注释"不暴露 setter"准确。类注释还声称"仅通过工厂方法 success()/error() 构造"，但实际存在 public 无参构造器和 public 三参构造器（L15-22）。该措辞与"仅通过工厂方法构造"略有出入，但属极轻微的文档措辞问题，无安全风险（无 setter 暴露，外部无法篡改 code/data）。

**Minor-1**：注释"仅通过工厂方法 success()/error() 构造"与存在 public 构造器略有出入。建议修正注释为"通过工厂方法或构造器构造，不暴露 setter"。（非阻塞）
...
**✅ 优点**：
- 构造器注入 `DemoService`（`@Autowired` 构造器 L43-46），符合 Spring 最佳实践
- 四接口路径与方法与契约完全一致：`/api/v1/helloworld`、`/api/v1/hash`、`/api/v1/bubble`、`/api/v1/export`
- `bubble` 接口参数解析含 try-catch（L71-75），非法参数抛 `ParamException`（非 `BusinessException`），正确映射 HTTP 400
- `export` 接口已校验 `format` 参数：`if (!"csv".equalsIgnoreCase(format)) throw new ParamException(...)`（L94-96）
- `export` 接口正确设置 `Content-Type: text/csv` + `Content-Disposition: attachment` + UTF-8 编码（L99-101）
- `export` 写流阶段 try-catch 防护（L102-111）：response 已 committed 时仅记日志，避免二次异常

### 3.8 DemoService.java — ✅ 实现正确

**✅ 优点**：
- `@Service` 注解（L22），被 Spring 管理
- 哈希用 `MessageDigest.getInstance("SHA-256")`（L124），UTF-8 编码，正确转十六进制
- **sha256 方法已使用 `HexFormat.of().formatHex(bytes)`**（L126，Java 17+），简洁高效，此前评审 Minor-3（`String.format("%02x", b)` 逐字节拼接）已不存在
- 冒泡排序逻辑正确（L51-61），含边界处理（空/默认数组兜底）
- `NoSuchAlgorithmException` 捕获后抛 `BusinessException(50000, ...)`（L128，5xxxx 类，正确映射 HTTP 500）
- 排序不修改入参（`new ArrayList<>(arr)` 副本，L49）
- `exportToCsv` 含 type 校验（L73-74）：空/null → `ParamException`，unknown → `ParamException`（L90）
...
共 **12 个** `@Test` 测试用例：
- 哈希：默认输入、自定义输入（2 个）
- 冒泡：默认数组、已排序数组、空输入兜底、逆序数组（4 个）
- 导出：all 多段完整性（含 hash 值 64 位校验、排序前后数组校验）、bubble、hash、helloworld（4 个）
- **异常路径已覆盖**：`exportToCsv("")` 和 `exportToCsv(null)` → `ParamException`；`exportToCsv("unknown")` → `ParamException`（2 个）
...
`errorThrower` 中 `code !== 0` 判失败，`errorHandler` 中 `Toast.show` 提示，不白屏。与需求一致。

### 4.3 src/services/demo.ts — ✅ 封装正确

`ApiResult<T>` 接口（L4-8）与后端 `ApiResult.java` 对齐。四个函数封装清晰。
...
**Minor-3**：`BubblePanel.tsx` L15-18 前端解析数组时 `.filter((n) => !Number.isNaN(n))` 会静默过滤掉非法输入（如 "abc"→NaN→过滤）。当用户输入 "5,abc,3" 时，实际发送 [5,3] 给后端。这与后端 `DemoController.bubble()` 的行为（抛 `ParamException` 报错）不一致——后端遇到非数字会报错，前端 mock 路径会静默过滤。建议前端遇到非法输入时给出提示而非静默过滤。（非阻塞，Demo 可接受）
...
| 编号 | 文件 | 问题 | 建议 |
|------|------|------|------|
| Minor-1 | `ApiResult.java` | 注释"仅通过工厂方法 success()/error() 构造"与存在 public 构造器略有出入（注意：类**无 setter**，注释"不暴露 setter"准确） | 修正注释为"通过工厂方法或构造器构造，不暴露 setter" |
| Minor-2 | `DemoService.java` | `exportToCsv` 始终用默认数据，不反映用户实际输入（已 README 文档化） | 后续迭代可改为 POST 接口携带请求体 |

> 注：前端 `BubblePanel.tsx` 的 `.filter((n) => !Number.isNaN(n))` 静默过滤非法输入行为已在 4.5 节记录，作为前端可选优化项，不单独编号入 Minor 清单。
...
| 单元测试 | ⏭️ 跳过 | 同上，测试代码静态审查通过（12 用例覆盖正向+异常路径） |
...
4. **P3（可选优化）**：BubblePanel 前端非法输入提示、mock hash 值动态化
...
剩余 2 个 Minor 为可选优化（ApiResult 注释措辞修正、导出语义 POST 化），不影响 Demo 功能正确性，可作为后续迭代项。不阻塞合并。
